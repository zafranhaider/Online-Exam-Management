...
def contact(request):
    return render(request,"exam/cont.html")
...
def contact_submit(request):
    if request.method == 'POST':
        ...
        en=Bug(name=name,email=email,web=web,phone=phone,message=message)
        en.save()
    return render(request, 'exam/cont.html')
...
def admbugg(request):
    if request.method == 'POST':
        # Check if a bug ID is provided for deletion
        bug_id_to_delete = request.POST.get('delete_bug_id')
        if bug_id_to_delete:
            try:
                bug_to_delete = Bug.objects.get(id=bug_id_to_delete)
                bug_to_delete.delete()
                # Redirect to the same page after deletion
                return redirect('admbug')
            except Bug.DoesNotExist:
                pass  # Handle the 
    bug_data = Bug.objects.all()
    return render(request, 'exam/admbug.html', {'bug_data': bug_data})
